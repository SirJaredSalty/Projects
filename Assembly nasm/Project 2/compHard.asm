...
;computer's move. The computer will try to block
...
compHard:
    ...
    ;Calls blockMove which checks if the
    ;computer can make a move to block the player
    ;from winning.
    mov     r15, moves
    call    blockMove
    cmp     al, [zero]
...
;Gets a random number if there's no move
;to block the player from winning.
getRandom:
    call    randomNum

compMove:
    ;Checks that the move isn't already taken.
    ;Gets a new number if it's taken.
    mov     [num_in], al
    xor     r14, r14
    mov     r14b, [computer]
    mov     r15, moves
